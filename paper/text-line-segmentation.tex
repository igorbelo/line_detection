...
\titulo{Método de segmentação de linhas de texto utilizando detecção de comunidades}

\autor{Igor Coura Belo}
\autorficha{Belo, Igor Coura}

\orientador{Prof. Me. Thiago M. Paixão}
...
\label{sec:deteccao-de-comunidades}

No estudo de redes complexas, geralmente representadas através de grafos, se diz que uma rede tem estrutura de comunidade quando os nós da rede podem ser agrupados em conjuntos de nós que estão densamente conectados internamente. Essa estrutura de comunidade tem aplicabilidade em diversas áreas - onde seus sistemas podem ser representados através de grafos -, como na sociológica por exemplo, onde comunidades podem representar características em comum entre pessoas como localização, língua, interesses, etc. [TODO: referência 1]

\begin{figure}[h]
  \centering
  \caption{Grafo com estrutura de comunidade.
  Três comunidades são identificadas nesse grafo, onde os nós estão densamente interconectados.}
  \label{fig:ex1}
  \newcount\mycount
  \includegraphics[scale=0.8]{network-community-structure.png}
  \fonte{Wikipedia}
\end{figure}

Existem diversos métodos e algoritmos desenvolvidos para a atividade de encontrar comunidades em uma rede arbitrária [TODO: referência 2]. O método utilizado nesse trabalho foi extraído de [citar o paper, autores?].

\subsection{O Algoritmo}
\label{sec:deteccao-de-comunidades-o-algoritmo}

O algoritmo consiste em decompor a rede em comunidades, sendo que a qualidade de cada decomposição é obtida através do cálculo da modularidade, um valor escalar entre -1 e 1 que representa a densidade das conexões da comunidade.

\begin{figure}[h]
  \centering
  \caption{Passo-a-passo do algoritmo de detecção de comunidades}
  \label{fig:ex2}
  \newcount\mycount
  \includegraphics[scale=0.5]{community-detection-algorithm.png}
  \fonte{Paper (citar?)}
  \nota{Cada \emph{pass} é composto por duas fases. A fase 1 (\emph{modularity optmization}) permite mudanças locais de comunidades sejam feitas. A fase 2 (\emph{community aggregation}) agrega as comunidades para construir uma nova rede. A nova rede é utilizada no próximo \emph{pass} e esse processo se repete até que não haja ganho de modularidade entre um \emph{pass} e outro.}
\end{figure}

Inicialmente o algoritmo define cada nó como uma comunidade isolada, o que torna o número de comunidades igual ao número de nós. Após isso, o algoritmo realiza o que é chamado de \emph{pass}, que consiste em duas fases.

Na primeira fase (\emph{Modularity Optimization}), o algoritmo irá percorrer cada nó verificando qual vizinho proporciona o maior ganho de modularidade (positiva), colocando os dois nós na mesma comunidade. Caso nenhum vizinho proporcione esse ganho, a comunidade inicial do nó é mantida.

\begin{minipage}{\linewidth}
\begin{lstlisting}[label={lst:deteccao-de-comunidade-o-algoritmo-fase-1}]
  G = grafo que representa a rede
  C[i] = comunidade do no i
  Q(i, j) = ganho de modularidade ao mover o no i para a
  comunidade do no j

  para cada no i em G:
    para cada no j vizinho de i:
      mod = Q(i, j)
      se mod > 0 and mod > Qmax:
        C[i] = C[j]
        Qmax = mod
\end{lstlisting}
\end{minipage}

A variação da modularidade ao mover um nó para uma determinada comunidade C é obtida através Equação ~(\ref{eq:variacao-de-modularidade}):

\begin{equation}
    \label{eq:variacao-de-modularidade}
    \Delta Q = \bigg[\dfrac{\sum in + k_i,i_n}{2m} - \bigg(\dfrac{\sum tot + k_i}{2m}\bigg)^2\bigg] - \bigg[\dfrac{\sum in}{2m} - \bigg(\dfrac{\sum tot}{2m}\bigg)^2 - \bigg(\dfrac{k_i}{2m}\bigg)^2\bigg],
\end{equation}

onde \( \sum in \) representa a soma dos pesos das conexões entre os nós contidos na comunidade C, \( \sum tot \) é a soma dos pesos das conexões a partir dos nós fora de C aos nós presentes em C, \(k_i\) é a soma dos pesos das conexões que chegam ao nó \emph{i}, \(k_i_,_i_n\) é a soma dos pesos das conexões que saem de \emph{i} para os nós da comunidade em C e \emph{m} é a soma dos pesos de todas as conexões do grafo.

Na segunda fase os nós de uma mesma comunidade são sintetizados em apenas um nó e o peso da ligação entre os novos nós é obtido através da soma dos pesos das ligações entre os nós das duas comunidades correspondentes.
